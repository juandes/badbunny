...
p <- ggplot(false.sequences, aes(x=lengths)) +
  ...
  annotate("text", x = mean(false.sequences$lengths) + 10, y = 50, 
           label = sprintf("Promedio: %.2f", mean(false.sequences$lengths)), size = 10) +
  bbc_style() +
  theme(axis.title = element_text(size = 20), 
        plot.margin = unit(c(1.0, 1.0, 1.0, 0.5), "cm"),
        axis.text.x = element_text(hjust = 1)) +
  labs(title="Días consecutivos en que no escuché a Bad Bunny",
       subtitle = "") +
...
```{r}
ggsave('../plots/hist_didnt_listened_ES.png', plot = p, 
...
p <- ggplot(true.sequences, aes(x=lengths)) +
  ...
  geom_vline(xintercept=mean(true.sequences$lengths), linetype="dashed", 
              size=1) +
  annotate("text", x = mean(true.sequences$lengths) + 2.5, y = 60, 
           label = sprintf("Promedio: %.2f", mean(true.sequences$lengths)), size = 10) +
  bbc_style() +
  theme(axis.title = element_text(size = 20), 
        plot.margin = unit(c(1.0, 1.0, 1.0, 0.5), "cm"),
        axis.text.x = element_text(hjust = 1)) +
  labs(title="Días consecutivos en que escuché a Bad Bunny",
       subtitle = "") +
  xlab("Días consecutivos") +
  ylab("Total")
print(p)
...
```{r}
ggsave('../plots/hist_did_listened_ES.png', plot = p, 
       width = 15, height = 8, units = 'in')
```
...
p <- ggplot(songs.by.day[songs.by.day$listened == TRUE,], aes(x=n)) +
  ...
  geom_vline(xintercept=mean(songs.by.day[songs.by.day$listened == TRUE,]$n), linetype="dashed", 
              size=1) +
  annotate("text", x = mean(songs.by.day[songs.by.day$listened == TRUE,]$n) + 15, y = 130, 
           label = sprintf("Promedio: %.2f", mean(songs.by.day[songs.by.day$listened == TRUE,]$n)), size = 10) +
  bbc_style() +
  theme(axis.title = element_text(size = 20), 
        plot.title = element_text(size = 18), 
        plot.margin = unit(c(1.0, 1.0, 1.0, 0.5), "cm"),
        axis.text.x = element_text(hjust = 1)) +
  labs(title="Número de canciones que reproducidas en los días en que escuché a Bad Bunny",
       subtitle = "") +
  xlab("Canciones reproducidas") +
  ylab("Total")
print(p)
```

```{r}
ggsave('../plots/hist_songs_listened_ES.png', plot = p, 
       width = 15, height = 8, units = 'in')
```
...
p <- ggplot(songs.by.hour, aes(x=reorder(hour, -n), y=n)) +
  ...
  labs(title="Canciones de Bad Bunny reproducidas por hora",
       subtitle = "De un total de 4477 canciones") +
...
```{r}
ggsave('../plots/songs_played_hour_ES.png', plot = p, 
       width = 15, height = 8, units = 'in')
```
...
```{r}
songs.by.weekday.hour.es <- songs.by.weekday.hour
songs.by.weekday.hour.es$weekdays <- factor(songs.by.weekday.hour.es$weekdays, levels = c("Monday", "Tuesday", "Wednesday", "Thursday",  "Friday", 
                                                 "Saturday", "Sunday"))

songs.by.weekday.hour.es$weekdays_en <- songs.by.weekday.hour.es$weekdays
songs.by.weekday.hour.es$weekdays_es <- songs.by.weekday.hour.es$weekdays
levels(songs.by.weekday.hour.es$weekdays_es)[1] <- "Lunes"
levels(songs.by.weekday.hour.es$weekdays_es)[2] <- "Martes"
levels(songs.by.weekday.hour.es$weekdays_es)[3] <- "Miercoles"
levels(songs.by.weekday.hour.es$weekdays_es)[4] <- "Jueves"
levels(songs.by.weekday.hour.es$weekdays_es)[5] <- "Viernes"
levels(songs.by.weekday.hour.es$weekdays_es)[6] <- "Sábado"
levels(songs.by.weekday.hour.es$weekdays_es)[7] <- "Domingo"

```

```{r}
p <- ggplot(songs.by.weekday.hour.es, aes(x = hour, y = weekdays_es)) +
  geom_point(aes(size = percentage ^ 2, color = n, stroke = n * 0.1)) +
  scale_colour_gradient(low = "#F0E442", high = "#CC79A7") + 
  labs(title="¿Cuándo escucho música de Bad Bunny?") +
  xlab('Hora') + ylab('Día de la semana') +
  modified_bbplot() +
  theme(axis.title = element_text(size = 24), 
        plot.margin = unit(c(1.0,1.5,1.0,1.0), 'cm'),
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) +
  labs(color='# Canciones\nreproducidas') +
  scale_size(guide = 'none')  # Supress the legend from aes "size."
print(p)
```

```{r}
ggsave('../plots/weekdays_hours_ES.png', plot = p, 
       width = 15, height = 8, units = 'in')
```
...
p <- ggplot(results, aes(x=scores)) +
  ...
  theme(axis.title = element_text(size = 20), 
        plot.margin = unit(c(1.0, 1.0, 1.0, 0.5), "cm"),
        axis.text.x = element_text(hjust = 1)) +
  labs(title="Histograma de las puntuaciones de anomalía producidas por el modelo",
       subtitle = "Una puntuación por canción reproducida") +
  xlab("Puntuaciones de anomalía") +
  ylab("Total")
print(p)
```
```{r}
ggsave('../plots/hist_scores_ES.png', plot = p, 
       width = 15, height = 8, units = 'in')
```
...
```{r}
summarized.results.with.times.es <- summarized.results.with.times
summarized.results.with.times.es$weekday <- factor(summarized.results.with.times.es$weekday, levels = c("Monday", "Tuesday", "Wednesday", "Thursday", 
                                                                                                  "Friday", "Saturday", "Sunday"))

levels(summarized.results.with.times.es$weekday)[1] <- "Lunes"
levels(summarized.results.with.times.es$weekday)[2] <- "Martes"
levels(summarized.results.with.times.es$weekday)[3] <- "Miercoles"
levels(summarized.results.with.times.es$weekday)[4] <- "Jueves"
levels(summarized.results.with.times.es$weekday)[5] <- "Viernes"
levels(summarized.results.with.times.es$weekday)[6] <- "Sábado"
levels(summarized.results.with.times.es$weekday)[7] <- "Domingo"
summarized.results.with.times.es$median.bool <- ifelse(summarized.results.with.times.es$median == 1, FALSE, TRUE)
```


```{r}
p <- ggplot(summarized.results.with.times.es, aes(x = hour, y = weekday)) +
  geom_point(aes(color = median.bool, size = 10)) +
  scale_colour_manual(values=c("#CC79A7", "#F0E442")) +
  #scale_colour_gradient(low = "#F0E442", high = "#CC79A7") + 
  labs(title="Tiempos anómalos") +
  xlab('Hora') + ylab('Día de la semana') +
  modified_bbplot() +
  theme(axis.title = element_text(size = 24), 
        plot.margin = unit(c(1.0,1.5,1.0,1.0), 'cm'),
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) +
  labs(color='¿Es una anomalía?') +
  scale_size(guide = 'none')  # Supress the legend from aes "size."
print(p)
```

```{r}
ggsave('../plots/anomalous_times_ES.png', plot = p, 
       width = 15, height = 8, units = 'in')
```
